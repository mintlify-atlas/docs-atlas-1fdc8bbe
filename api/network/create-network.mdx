...
<ResponseField name="net" type="Network">
  ...
  **Network struct public fields:**
  - `Inputs` (int): Number of input neurons (read-only after creation)
  - `Outputs` (int): Number of output neurons (read-only after creation)
  - `HiddenWeights` (*mat.Dense): Weight matrix between input and hidden layer (200×784 for MNIST)
  - `OutputWeights` (*mat.Dense): Weight matrix between hidden and output layer (10×200 for MNIST)
  - `learningRate` (float64): Learning rate used during training (unexported, set at creation)
</ResponseField>
